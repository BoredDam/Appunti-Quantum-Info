...
\newpage
\section{Teorema del Codice di un Canale Rumoroso}
...
\subsection{$(N,K)$ codici a blocco}
...
\subsection{Tasso di comunicazione}
...
\subsection{Probabilità di un errore di blocco}
...
\subsection{Probabilità massima di un errore di blocco}
La probabilità \textbf{massima} di un errore di blocco è:
\begin{equation}
	p_{BM} = \max_{s_{in}} P(s_{out} \neq s_{in}|s_{in})
\end{equation}

\subsection{Probabilità di errore di bit}
Assumendo che $s$ sia rappresentato da un vettore binario \textbf{s} di lunghezza $K$, la probabilità di bit error consiste nella probabilità media che un bit dell’output sia
diverso dal corrispondente bit dell’input.
\begin{equation}
	p_b = \max_{s} \left(\frac{1}{k} \sum_{i=1}^{k} Pr[\hat{s}(i) \neq s(i)|s]\right)
\end{equation}

\subsection{Decoder Ottimale}
...
\newpage
\section{Teorema di Shannon per canali rumorosi}
...
\subsection{Conferma del Teorema di Shannon per canali rumorosi sul Noisy Typewriter Channel}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.75\textwidth]{images/proofNT.png}
\end{figure}
